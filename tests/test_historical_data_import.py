...
from uci_points_model.historical_data_import import (
...
def _build_minimal_upstream_source(source_root: Path) -> None:
    ...
    _write_csv(
        ...
        [
            ...
            {
                "prior_team_slug": "team-2022",
                "next_team_slug": "team-2023",
                "prior_n_riders_150": 5,
                "next_top5": 0,
            },
        ],
    ...
    _write_csv(
        ...
        [
            ...
            {"check_name": "share_sanity", "status": "pass", "value": 0.0, "notes": ""},
        ],
...
def test_import_historical_proteam_data_copies_and_validates_local_source(tmp_path: Path) -> None:
    ...
    assert len(result.imported_files) == 6
    assert (import_root / "import_metadata.json").exists()

    metadata = json.loads((import_root / "import_metadata.json").read_text())
    assert metadata["source_mode"] == "local"
    assert metadata["validation_report"]["passed"] is True

    team_panel = pd.read_csv(imported_dataset_path("historical_proteam_team_panel", import_root))
    assert sorted(team_panel["season_year"].tolist()) == [2021, 2022, 2023, 2024, 2025, 2026]


def test_validate_historical_import_rejects_missing_required_columns(tmp_path: Path) -> None:
    source_root = tmp_path / "upstream"
    import_root = tmp_path / "imported"
    _build_minimal_upstream_source(source_root)

    broken_team_panel = source_root / "data" / "historical_proteam_team_panel.csv"
    pd.read_csv(broken_team_panel).drop(columns=["n_riders_150"]).to_csv(broken_team_panel, index=False)
    ...
    report = validate_historical_import(import_root=import_root)

    assert report.passed is False
    issues = report.file_results["historical_proteam_team_panel"].issues
    assert any("n_riders_150" in issue for issue in issues)


def test_import_historical_proteam_data_include_optional_imports_optional_files(tmp_path: Path) -> None:
    source_root = tmp_path / "upstream"
    import_root = tmp_path / "imported"
    _build_minimal_upstream_source(source_root)
    ...
    assert len(result.imported_files) == 10
    assert imported_dataset_path("rider_season_result_summary", import_root).exists()
    assert imported_dataset_path("rider_transfer_context_enriched", import_root).exists()
    assert imported_dataset_path("race_entries_pts_v2", import_root).exists()


def test_import_historical_proteam_data_raises_when_local_source_is_missing(tmp_path: Path) -> None:
    missing_root = tmp_path / "missing-upstream"

    try:
        import_historical_proteam_data(source_root=missing_root, import_root=tmp_path / "imported")
    except HistoricalImportError:
        pass
    else:
        raise AssertionError("Expected HistoricalImportError when local source root is missing.")
