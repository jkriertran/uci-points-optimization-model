...
from uci_points_model.calendar_ev import (
    ...
    load_team_profile,
    load_team_reference_total,
    summarize_team_calendar_ev,
)
from uci_points_model.team_calendar import build_live_team_calendar


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build deterministic team calendar expected value outputs.")
    parser.add_argument("--team-slug", required=True, help="Team slug with season year.")
...
def _load_calendar_or_build(team_slug: str, planning_year: int, calendar_path: Path, as_of_date: str | None) -> pd.DataFrame:
...
def _stack_summaries(summary_tables: dict[str, pd.DataFrame], team_reference_total: float | None) -> pd.DataFrame:
    overview_df = summary_tables["overview"].copy()
    overview_df["summary_view"] = "overview"
    overview_df["dimension"] = "all"
    overview_df["dimension_value"] = "all"
    if team_reference_total is not None:
        overview_df["team_reference_total_points"] = team_reference_total
        overview_df["actual_points_capture_ratio"] = overview_df["actual_points_known"] / team_reference_total

    by_month_df = summary_tables["by_month"].copy()
    by_month_df["summary_view"] = "by_month"
    by_month_df["dimension"] = "month"
    by_month_df["dimension_value"] = by_month_df["month"].astype(str)

    by_category_df = summary_tables["by_category"].copy()
    by_category_df["summary_view"] = "by_category"
    by_category_df["dimension"] = "category"
    by_category_df["dimension_value"] = by_category_df["category"].astype(str)

    return pd.concat([overview_df, by_month_df, by_category_df], ignore_index=True, sort=False)
...
def _write_dictionary(path: Path) -> None:
    ...
    lines = ["# Team Calendar EV Data Dictionary", "", "| Field | Description |", "| --- | --- |"]
    lines.extend(f"| `{field}` | {description} |" for field, description in dictionary_rows)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n")
...
def main() -> None:
    ...
    team_calendar_df = _load_calendar_or_build(args.team_slug, args.planning_year, calendar_path, args.as_of_date)
    ...
    summary_tables = summarize_team_calendar_ev(calendar_ev_df)
    stacked_summary_df = _stack_summaries(summary_tables, load_team_reference_total(args.team_slug))
    ...
    stacked_summary_df.to_csv(summary_output_path, index=False)
    _write_readme(readme_path, args.team_slug, args.planning_year)
    ...
    print(f"Wrote {len(stacked_summary_df)} summary rows to {summary_output_path}")
