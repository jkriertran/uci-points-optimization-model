...
from .top5_proteam_model import (
    ...
    DEFAULT_TOP5_BASELINE_SPECS,
    TOP5_TARGET_COLUMN,
    default_top5_proteam_model_output_root,
    evaluate_top5_proteam_expanding_window,
    fit_top5_proteam_baseline,
)

TOP5_BACKTEST_SUMMARY_FILENAME = "top5_proteam_backtest_summary.json"
TOP5_BACKTEST_BENCHMARK_FILENAME = "top5_proteam_backtest_benchmark.csv"
TOP5_BACKTEST_FOLDS_FILENAME = "top5_proteam_backtest_folds.csv"
TOP5_BACKTEST_PREDICTIONS_FILENAME = "top5_proteam_backtest_predictions.csv"
TOP5_BACKTEST_REPORT_FILENAME = "top5_proteam_backtest_report.md"


@dataclass(frozen=True, slots=True)
class Top5ProTeamBacktestArtifacts:
...
def build_top5_proteam_backtest_artifacts(
    ...
    *,
...
) -> Top5ProTeamBacktestArtifacts:
    if not model_specs:
        raise ValueError("At least one top-five ProTeam backtest model spec is required.")
    ...
    if prepared.empty:
        raise ValueError("Top-five ProTeam backtest requires observed target rows.")
    ...
    for model_name, feature_columns in model_specs:
        fit_result = fit_top5_proteam_baseline(
            prepared,
        ...
        backtest_summary, fold_summaries, prediction_table = evaluate_top5_proteam_expanding_window(
            prepared,
            feature_columns=feature_columns,
            model_name=model_name,
            target_column=target_column,
    ...
    if not prediction_table.empty:
        prediction_table = prediction_table.sort_values(
            ["model_name", "test_next_season", "predicted_probability_rank", "prior_team_name"],
            na_position="last",
        ).reset_index(drop=True)

    winning_model_name = benchmark_table.iloc[0]["model_name"] if not benchmark_table.empty else None
    summary = {
        "artifact_version": "top5_proteam_backtest_v1",
    ...
    return Top5ProTeamBacktestArtifacts(
        summary=summary,
...
def write_top5_proteam_backtest_artifacts(
    artifacts: Top5ProTeamBacktestArtifacts,
    *,
